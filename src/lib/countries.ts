...
export const isoToFlag = (iso2: string): string => {
  ...
  const chars = iso2.toUpperCase().split('').map((c) => String.fromCodePoint(base + (c.charCodeAt(0) - A)))
  return chars.join('')
}

// Twemoji CDN URL for a country flag (SVG preferred for crisp rendering)
export const isoToTwemojiUrl = (iso2: string, kind: 'svg' | 'png' = 'svg'): string => {
  if (!iso2 || iso2.length !== 2) return ''
  const base = 0x1f1e6
  const A = 'A'.charCodeAt(0)
  const hi = base + (iso2[0].toUpperCase().charCodeAt(0) - A)
  const lo = base + (iso2[1].toUpperCase().charCodeAt(0) - A)
  const filename = `${hi.toString(16)}-${lo.toString(16)}`
  if (kind === 'svg') {
    return `https://cdnjs.cloudflare.com/ajax/libs/twemoji/14.0.2/svg/${filename}.svg`
  }
  return `https://cdnjs.cloudflare.com/ajax/libs/twemoji/14.0.2/72x72/${filename}.png`
}

// A practical, lightweight set of countries and dial codes.
// Extend as needed.
...
export const topCountryIso2 = ['US', 'GB', 'CA', 'AU', 'IN', 'NG', 'ZA']

export const byIso = (iso2: string): Country | undefined =>
  countries.find((c) => c.iso2 === iso2.toUpperCase())

export const byDialCode = (dial: string): Country[] =>
  countries.filter((c) => c.dialCode === (dial.startsWith('+') ? dial : `+${dial}`))

// Comprehensive list of countries and territories with dial codes
// Source: aggregated from common dialing code references (libphonenumber, ITU)
// Note: Some regions share the same country code (e.g., +1 NANP, +44 UK territories)
export const countriesFull: Country[] = [
  ...
  { name: 'Congo (Congo-Brazzaville)', iso2: 'CG', dialCode: '+242' },
